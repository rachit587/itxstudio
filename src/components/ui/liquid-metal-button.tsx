...
export function LiquidMetalButton({
  ...
  useEffect(() => {
    ...
    const loadShader = async () => {
      ...
      } catch (error) { console.error("Failed to load shader:", error); }
    };
    loadShader();
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (shaderMount.current?.setSpeed) {
      ...
      setTimeout(() => { if (isHovered) shaderMount.current?.setSpeed?.(1); else shaderMount.current?.setSpeed?.(0.6); }, 300);
    }
    ...
    <div className="relative inline-block">
      <div style={{ perspective: "1000px", perspectiveOrigin: "50% 50%" }}>
        <div style={{ position: "relative", width: `${dimensions.width}px`, height: `${dimensions.height}px`, transformStyle: "preserve-3d", transition: "all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, width: `${dimensions.width}px`, height: `${dimensions.height}px`, transform: `translateZ(0px)`, zIndex: 10 }}>
            <div style={{ height: `${dimensions.height}px`, width: `${dimensions.width}px`, borderRadius: "100px", boxShadow: isHovered ? "0px 0px 0px 1px rgba(0,255,102,0.4), 0px 8px 20px 0px rgba(0,255,102,0.15)" : "0px 0px 0px 1px rgba(0,0,0,0.3), 0px 9px 9px 0px rgba(0,0,0,0.12)" }}>
          ...
          <button ref={buttonRef} onClick={handleClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onMouseDown={() => setIsPressed(true)} onMouseUp={() => setIsPressed(false)}
            style={{ position: "absolute", top: 0, left: 0, width: `${dimensions.width}px`, height: `${dimensions.height}px`, background: "transparent", border: "none", cursor: "pointer", zIndex: 40, transform: "translateZ(25px)", borderRadius: "100px", overflow: "hidden" }} aria-label={label}>
            {ripples.map(ripple => (
